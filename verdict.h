...
/* quality functions for hex elements */
    ...
    VERDICT_EXPORT double hex_edge_ratio( int num_nodes, double coordinates[][3] );
    ...
    VERDICT_EXPORT double hex_max_edge_ratio( int num_nodes, double coordinates[][3] );

    //! Calculates hex skew metric. 
    /** Maximum |cos A| where A is the angle between edges at hex center.   
      Reference --- L.M. Taylor, and D.P. Flanagan, Pronto3D - A Three Dimensional Transient
         Solid Dynamics Program, SAND87-1912, Sandia National Laboratories, 1989. */
    VERDICT_EXPORT double hex_skew( int num_nodes, double coordinates[][3] );

    //! Calculates hex taper metric 
    /**  Maximum ratio of lengths derived from opposite edges. 
      Reference --- L.M. Taylor, and D.P. Flanagan, Pronto3D - A Three Dimensional Transient
         Solid Dynamics Program, SAND87-1912, Sandia National Laboratories, 1989. */
    VERDICT_EXPORT double hex_taper( int num_nodes, double coordinates[][3] );

    //! Calculates hex volume 
    /**  Jacobian at hex center. 
      Reference --- L.M. Taylor, and D.P. Flanagan, Pronto3D - A Three Dimensional Transient
         Solid Dynamics Program, SAND87-1912, Sandia National Laboratories, 1989. */
    VERDICT_EXPORT double hex_volume( int num_nodes, double coordinates[][3] );

    //! Calculates hex stretch metric   
    /**  Sqrt(3) * minimum edge length / maximum diagonal length. 
      Reference --- FIMESH code */ 
    VERDICT_EXPORT double hex_stretch( int num_nodes, double coordinates[][3] );

    //! Calculates hex diagonal metric   
    /** Minimum diagonal length / maximum diagonal length. 
      Reference --- Unknown */ 
    VERDICT_EXPORT double hex_diagonal( int num_nodes, double coordinates[][3] );
    ...
    VERDICT_EXPORT double hex_med_aspect_frobenius( int num_nodes, double coordinates[][3] );
    ...
    VERDICT_EXPORT double hex_max_aspect_frobenius( int num_nodes, double coordinates[][3] );
    //! Calculates hex condition metric. This is a synonym for \ref hex_max_aspect_frobenius.
    VERDICT_EXPORT double hex_condition( int num_nodes, double coordinates[][3] );
    ...
    /** Minimum pointwise volume of local map at 8 corners & center of hex. 
       ...
       Optimization of the Jacobian Matrix Norm and Associated Quantities, 
       Intl. J. Numer. Meth. Engng. 2000, 48:1165-1185. */ 
    VERDICT_EXPORT double hex_jacobian( int num_nodes, double coordinates[][3] );
    ...
    VERDICT_EXPORT double hex_shear( int num_nodes, double coordinates[][3] );

    //! Calculates hex shape metric.
    /** 3/Mean Ratio of weighted Jacobian matrix. 
       Reference --- P. Knupp, Algebraic Mesh Quality Metrics for
       Unstructured Initial Meshes, submitted for publication.  */
    VERDICT_EXPORT double hex_shape( int num_nodes, double coordinates[][3] );
    ...
    VERDICT_EXPORT double hex_relative_size_squared( int num_nodes, double coordinates[][3], double average_hex_volume );

    //! Calculates hex shape-size metric.
    /** Product of Shape and Relative Size.
       Reference --- P. Knupp, Algebraic Mesh Quality Metrics for
       Unstructured Initial Meshes, submitted for publication.  */
    VERDICT_EXPORT double hex_shape_and_size( int num_nodes, double coordinates[][3], double average_hex_volume );

    //! Calculates hex shear-size metric   
    /** Product of Shear and Relative Size.
       Reference --- P. Knupp, Algebraic Mesh Quality Metrics for
       Unstructured Initial Meshes, submitted for publication.  */
    VERDICT_EXPORT double hex_shear_and_size( int num_nodes, double coordinates[][3], double average_hex_volume );
...
/* quality functions for tet elements */
    ...
    /**  Hmax / Hmin where Hmax and Hmin are respectively the maximum and the
       minimum edge lengths */ 
    VERDICT_EXPORT double tet_edge_ratio( int num_nodes, double coordinates[][3] );
    ...
    VERDICT_EXPORT double tet_aspect_ratio( int num_nodes, double coordinates[][3] );

    //! Calculates tet aspect gamma metric.
    /**  Srms**3 / (8.479670*V) where Srms = sqrt(Sum(Si**2)/6), Si = edge length. 
       Reference ---  V. N. Parthasarathy et al, A comparison of tetrahedron 
       quality measures, Finite Elem. Anal. Des., Vol 15(1993), 255-261. */ 
    VERDICT_EXPORT double tet_aspect_gamma( int num_nodes, double coordinates[][3] );
    ...
    /** Frobenius condition number when the reference element is regular
       ...
       Optimization of the Jacobian Matrix Norm and Associated Quantities,
       Intl. J. Numer. Meth. Engng. 2000, 48:1165-1185. */
    VERDICT_EXPORT double tet_aspect_frobenius( int num_nodes, double coordinates[][3] );

    //! Calculates tet minimum dihedral angle.
    /** Minimum (nonoriented) dihedral angle of a tetrahedron, expressed in degrees. */
    VERDICT_EXPORT double tet_minimum_angle( int num_nodes, double coordinates[][3] );

    //! Calculates tet collapse ratio metric.
    /**  Collapse ratio */ 
    VERDICT_EXPORT double tet_collapse_ratio( int num_nodes, double coordinates[][3] );

    //! Calculates tet volume.
    /** (1/6) * Jacobian at corner node.
       Reference ---  V. N. Parthasarathy et al, A comparison of tetrahedron 
       quality measures, Finite Elem. Anal. Des., Vol 15(1993), 255-261. */ 
    VERDICT_EXPORT double tet_volume( int num_nodes, double coordinates[][3] );
    ...
    /** Condition number of the Jacobian matrix at any corner. 
       ...
       Optimization of the Jacobian Matrix Norm and Associated Quantities,
       Intl. J. Numer. Meth. Engng. 2000, 48:1165-1185. */
    VERDICT_EXPORT double tet_condition( int num_nodes, double coordinates[][3] );
    ...
    /** Minimum pointwise volume at any corner. 
       ...
       Optimization of the Jacobian Matrix Norm and Associated Quantities,
       Intl. J. Numer. Meth. Engng. 2000, 48:1165-1185. */
    VERDICT_EXPORT double tet_jacobian( int num_nodes, double coordinates[][3] );
    ...
    /** 3/Mean Ratio of weighted Jacobian matrix.
       Reference --- P. Knupp, Algebraic Mesh Quality Metrics for
       Unstructured Initial Meshes, submitted for publication. */ 
    VERDICT_EXPORT double tet_shape( int num_nodes, double coordinates[][3] );

    //! Calculates tet relative size metric.
    /** Min( J, 1/J ), where J is determinant of weighted Jacobian matrix.
       Reference --- P. Knupp, Algebraic Mesh Quality Metrics for
       Unstructured Initial Meshes, submitted for publication. */ 
    VERDICT_EXPORT double tet_relative_size_squared( int num_nodes, double coordinates[][3], double average_tet_size );

    //! Calculates tet shape-size metric.
    /** Product of Shape and Relative Size. 
       Reference --- P. Knupp, Algebraic Mesh Quality Metrics for
       Unstructured Initial Meshes, submitted for publication. */ 
    VERDICT_EXPORT double tet_shape_and_size( int num_nodes, double coordinates[][3], double average_tet_size );

    //! Calculates tet distortion metric.
    /** {min(|J|)/actual volume}*parent volume, parent volume = 1/6 for tet.
       Reference --- SDRC/IDEAS Simulation: Finite Element Modeling--User's Guide */ 
    VERDICT_EXPORT double tet_distortion( int num_nodes, double coordinates[][3] );

    //! Calculates tet equivolume skew metric.
    VERDICT_EXPORT double tet_equivolume_skew( int num_nodes, double coordinates[][3] );

    //! Calculates tet squish index metric.
    VERDICT_EXPORT double tet_squish_index( int num_nodes, double coordinates[][3] );

    //! Calculates tet equiangle skew metric.
    VERDICT_EXPORT double tet_equiangle_skew( int num_nodes, double coordinates[][3] );
    
/* quality functions for pyramid elements */ 

    //! Calculates pyramid volume.
    VERDICT_EXPORT double pyramid_volume( int num_nodes, double coordinates[][3] );
    //! Caluculates pyramid jacaboian based on bisecting into two tets
    VERDICT_EXPORT double pyramid_jacobian( int num_nodes, double coordinates[][3] );

    //! Calculates pyramid scaled jacaboian based on bisecting into two tets
    VERDICT_EXPORT double pyramid_scaled_jacobian( int num_nodes, double coordinates[][3] );
    ...
    VERDICT_EXPORT double pyramid_shape( int num_nodes, double coordinates[][3] );

    //! Calculates the pyramid equiangle skew metric.
    VERDICT_EXPORT double pyramid_equiangle_skew( int num_nodes, double coordinates[][3] );


/* quality functions for wedge elements */

    //! Calculates wedge volume.
    VERDICT_EXPORT double wedge_volume( int num_nodes, double coordinates[][3] );

    //! Calculates wedge edge ratio metric.
    /**  Hmax / Hmin where Hmax and Hmin are respectively the maximum and the
       minimum edge lengths */
    VERDICT_EXPORT double wedge_edge_ratio( int num_nodes, double coordinates[][3] );
    ...
    VERDICT_EXPORT double wedge_max_aspect_frobenius( int num_nodes, double coordinates[][3] );
    ...
    VERDICT_EXPORT double wedge_shape( int num_nodes, double coordinates[][3] );

    //! Calculates wedge max aspect forbenius.
    /** max(F_0123, F_1204, F_2015, F_3540, F_4351, F_5432)
      Reference --- Adaptation of hex max aspect frobenius */
    VERDICT_EXPORT double wedge_condition( int num_nodes, double coordinates[][3] );

    //! Calculates wedge equiangle skew metric
    VERDICT_EXPORT double wedge_equiangle_skew( int num_nodes, double coordinates[][3] );


/* quality functions for knife elements */

    //! Calculates knife volume.
    VERDICT_EXPORT double knife_volume( int num_nodes, double coordinates[][3] );

    
/* quality functions for edge elements */

    //! Calculates edge length. 
    VERDICT_EXPORT double edge_length( int num_nodes, double coordinates[][3] );
...
/* quality functions for quad elements */
    ...
    VERDICT_EXPORT double quad_edge_ratio( int num_nodes, double coordinates[][3] );
    ...
    VERDICT_EXPORT double quad_max_edge_ratio( int num_nodes, double coordinates[][3] );

    //! Calculates quad aspect ratio
    /** aspect ratio
        Reference --- P. P. Pebay, Planar Quadrangle Quality
        Measures, Eng. Comp., 2004, 20(2):157-173 */
    VERDICT_EXPORT double quad_aspect_ratio( int num_nodes, double coordinates[][3] );

    //! Calculates quad radius ratio
    /** radius ratio
        Reference --- P. P. Pebay, Planar Quadrangle Quality
        Measures, Eng. Comp., 2004, 20(2):157-173 */
    VERDICT_EXPORT double quad_radius_ratio( int num_nodes, double coordinates[][3] );

    //! Calculates quad average Frobenius aspect
    /** average Frobenius aspect
        Reference --- P. P. Pebay, Planar Quadrangle Quality
        Measures, Eng. Comp., 2004, 20(2):157-173 */
    VERDICT_EXPORT double quad_med_aspect_frobenius( int num_nodes, double coordinates[][3] );

    //! Calculates quad maximum Frobenius aspect
    /** average Frobenius aspect
        Reference --- P. P. Pebay, Planar Quadrangle Quality
        Measures, Eng. Comp., 2004, 20(2):157-173 */
    VERDICT_EXPORT double quad_max_aspect_frobenius( int num_nodes, double coordinates[][3] );

    //! Calculates quad skew metric.
    /** Maximum |cos A| where A is the angle between edges at quad center. 
       Reference --- J. Robinson, CRE Method of element testing and the 
       Jacobian shape parameters, Eng. Comput., Vol 4, 1987. */ 
    VERDICT_EXPORT double quad_skew( int num_nodes, double coordinates[][3] );

    //! Calculates quad taper metric.
    /** Maximum ratio of lengths derived from opposite edges. 
       Reference --- J. Robinson, CRE Method of element testing and the 
       Jacobian shape parameters, Eng. Comput., Vol 4, 1987. */ 
    VERDICT_EXPORT double quad_taper( int num_nodes, double coordinates[][3] );

    //! Calculates quad warpage metric.
    /** Cosine of Minimum Dihedral Angle formed by Planes Intersecting in Diagonals. 
       Reference --- J. Robinson, CRE Method of element testing and the 
       Jacobian shape parameters, Eng. Comput., Vol 4, 1987. */ 
    VERDICT_EXPORT double quad_warpage( int num_nodes, double coordinates[][3] );

    //! Calculates quad area.
    /** Jacobian at quad center.
       Reference --- J. Robinson, CRE Method of element testing and the 
       Jacobian shape parameters, Eng. Comput., Vol 4, 1987. */ 
    VERDICT_EXPORT double quad_area( int num_nodes, double coordinates[][3] );

    //! Calculates quad strech metric.
    /** Sqrt(2) * minimum edge length / maximum diagonal length.
       Reference --- FIMESH code. */
    VERDICT_EXPORT double quad_stretch( int num_nodes, double coordinates[][3] );

    //! Calculates quad's smallest angle.
    /** Smallest included quad angle (degrees).
       Reference --- Unknown. */ 
    VERDICT_EXPORT double quad_minimum_angle( int num_nodes, double coordinates[][3] );

    //! Calculates quad's largest angle.
    /** Largest included quad angle (degrees). 
       Reference --- Unknown. */ 
    VERDICT_EXPORT double quad_maximum_angle( int num_nodes, double coordinates[][3] );

    //! Calculates quad oddy metric.
    VERDICT_EXPORT double quad_oddy( int num_nodes, double coordinates[][3] );
    ...
    /** Maximum condition number of the Jacobian matrix at 4 corners.
       ...
       Optimization of the Jacobian Matrix Norm and Associated Quantities,
       Intl. J. Numer. Meth. Engng. 2000, 48:1165-1185. */
    VERDICT_EXPORT double quad_condition( int num_nodes, double coordinates[][3] );
    ...
    /** Minimum pointwise volume of local map at 4 corners & center of quad. 
       ...
       Optimization of the Jacobian Matrix Norm and Associated Quantities,
       Intl. J. Numer. Meth. Engng. 2000, 48:1165-1185. */
    VERDICT_EXPORT double quad_jacobian( int num_nodes, double coordinates[][3] );
    ...
    /** Minimum Jacobian divided by the lengths of the 2 edge vectors. 
       ...
       Optimization of the Jacobian Matrix Norm and Associated Quantities,
       Intl. J. Numer. Meth. Engng. 2000, 48:1165-1185. */
    VERDICT_EXPORT double quad_scaled_jacobian( int num_nodes, double coordinates[][3] );

    //! Calculates quad shear metric.
    /** 2/Condition number of Jacobian Skew matrix.
       Reference --- P. Knupp, Algebraic Mesh Quality Metrics for
       Unstructured Initial Meshes, submitted for publication. */
    VERDICT_EXPORT double quad_shear( int num_nodes, double coordinates[][3] );

    //! Calculates quad shape metric.
    /** 2/Condition number of weighted Jacobian matrix. 
       Reference --- P. Knupp, Algebraic Mesh Quality Metrics for
       Unstructured Initial Meshes, submitted for publication. */
    VERDICT_EXPORT double quad_shape( int num_nodes, double coordinates[][3] );

    //! Calculates quad relative size metric.
    /** Min( J, 1/J ), where J is determinant of weighted Jacobian matrix. 
       Reference --- P. Knupp, Algebraic Mesh Quality Metrics for
       Unstructured Initial Meshes, submitted for publication. */
    VERDICT_EXPORT double quad_relative_size_squared( int num_nodes, double coordinates[][3], double average_quad_area );

    //! Calculates quad shape-size metric.
    /** Product of Shape and Relative Size. 
       Reference --- P. Knupp, Algebraic Mesh Quality Metrics for
       Unstructured Initial Meshes, submitted for publication. */
    VERDICT_EXPORT double quad_shape_and_size( int num_nodes, double coordinates[][3], double average_quad_area );

    //! Calculates quad shear-size metric.
    /** Product of Shear and Relative Size. 
       Reference --- P. Knupp, Algebraic Mesh Quality Metrics for
       Unstructured Initial Meshes, submitted for publication. */
    VERDICT_EXPORT double quad_shear_and_size( int num_nodes, double coordinates[][3], double average_quad_area );

    //! Calculates quad distortion metric.
    /** {min(|J|)/actual area}*parent area, parent area = 4 for quad.
       Reference --- SDRC/IDEAS Simulation: Finite Element Modeling--User's Guide */
    VERDICT_EXPORT double quad_distortion( int num_nodes, double coordinates[][3] );

    //! Calculates the quad equiangle skew
    VERDICT_EXPORT double quad_equiangle_skew( int num_nodes, double coordinates[][3] );
...
/* quality functions for triangle elements */
    ...
    VERDICT_EXPORT double tri_edge_ratio( int num_nodes, double coordinates[][3] );

    //! Calculates triangle metric.
    /** aspect ratio
        Reference --- P. P. Pebay & T. J. Baker, Analysis of Triangle Quality
        Measures, AMS Math. Comp., 2003, 72(244):1817-1839 */
    VERDICT_EXPORT double tri_aspect_ratio( int num_nodes, double coordinates[][3] );

    //! Calculates triangle metric.
    /** radius ratio
        Reference --- P. P. Pebay & T. J. Baker, Analysis of Triangle Quality
        Measures, AMS Math. Comp., 2003, 72(244):1817-1839 */
    VERDICT_EXPORT double tri_radius_ratio( int num_nodes, double coordinates[][3] );

    //! Calculates triangle metric.
    /** Frobenius aspect */
    VERDICT_EXPORT double tri_aspect_frobenius( int num_nodes, double coordinates[][3] );

    //! Calculates triangle metric.
    /** Maximum included angle in triangle */
    VERDICT_EXPORT double tri_area( int num_nodes, double coordinates[][3] );

    //! Calculates triangle metric.
    /** Minimum included angle in triangle */
    VERDICT_EXPORT double tri_minimum_angle( int num_nodes, double coordinates[][3] );

    //! Calculates triangle metric.
    /** Maximum included angle in triangle */
    VERDICT_EXPORT double tri_maximum_angle( int num_nodes, double coordinates[][3] );
    ...
    /** Condition number of the Jacobian matrix.
       ...
       Optimization of the Jacobian Matrix Norm and Associated Quantities,
       Intl. J. Numer. Meth. Engng. 2000, 48:1165-1185. */ 
    VERDICT_EXPORT double tri_condition( int num_nodes, double coordinates[][3] );
    ...
    /** Minimum Jacobian divided by the lengths of 2 edge vectors. 
       ...
       Optimization of the Jacobian Matrix Norm and Associated Quantities,
       Intl. J. Numer. Meth. Engng. 2000, 48:1165-1185. */ 
    VERDICT_EXPORT double tri_scaled_jacobian( int num_nodes, double coordinates[][3] );

    //! Calculates triangle metric.
    /**  */
    VERDICT_EXPORT double tri_shear( int num_nodes, double coordinates[][3] );
    
    //! Calculates triangle metric.
    /** Min( J, 1/J ), where J is determinant of weighted Jacobian matrix. 
       Reference ---  P. Knupp, Algebraic Mesh Quality Metrics for
       Unstructured Initial Meshes, submitted for publication. */
    VERDICT_EXPORT double tri_relative_size_squared( int num_nodes, double coordinates[][3], double average_tri_area );

    //! Calculates triangle metric.
    /** 2/Condition number of weighted Jacobian matrix. 
       Reference ---  P. Knupp, Algebraic Mesh Quality Metrics for
       Unstructured Initial Meshes, submitted for publication. */
    VERDICT_EXPORT double tri_shape( int num_nodes, double coordinates[][3] );

    //! Calculates triangle metric.
    /**  Product of Shape and Relative Size. 
       Reference ---  P. Knupp, Algebraic Mesh Quality Metrics for
       Unstructured Initial Meshes, submitted for publication. */
    VERDICT_EXPORT double tri_shape_and_size( int num_nodes, double coordinates[][3], double average_tri_area );

    //! Calculates triangle metric.
    /** {min(|J|)/actual area}*parent area, parent area = 1/2 for triangular element. 
       Reference --- SDRC/IDEAS Simulation: Finite Element Modeling--User's Guide */
    VERDICT_EXPORT double tri_distortion( int num_nodes, double coordinates[][3] );
